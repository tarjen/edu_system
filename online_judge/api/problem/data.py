...
def update_problem_data(problem_id):
    ...
    if current_user.id != problem.user_id:
        return jsonify({"error": "the user is not permitted to update the problem"}), 404
    ...
    if uploaded_file.filename == '':
        return jsonify({"error": "empty filename"}), 404

    # 验证文件扩展名
    if not uploaded_file.filename.lower().endswith('.zip'):
        return jsonify({"error": "the upload_file is not .zip"}), 404
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if not zipfile.is_zipfile(temp_zip):
                return jsonify({"error": "the zip file is not available"}), 404
            ...
            for f in os.listdir(extract_dir):
                if re.match(r'^\d+\.in$', f):
                    case_id = f.split('.')[0]
                    ans_file = f"{case_id}.ans"
                    if not os.path.exists(os.path.join(extract_dir, ans_file)):
                        return jsonify({"error": f"testcase_{case_id}缺少.ans"}), 404
                    testcases.append(int(case_id))

            # 检查测试点数量和连续性
            testcases.sort()
            if not testcases:
                return jsonify({"error": "未找到有效测试数据"}), 404
                
            if testcases[0] != 1 or testcases[-1] > 30:
                return jsonify({"error": "测试点编号需从1开始且不超过30"}), 404

            if testcases != list(range(1, len(testcases)+1)):
                return jsonify({"error": "测试点编号不连续"}), 404
        ...
        return jsonify({"error": "文件权限错误"}), 404
